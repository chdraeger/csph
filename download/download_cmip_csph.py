...
#=======================================================================================================================
def get_cmip6(var, table_id, source_id, experiment_id, member_id, start_day, end_day, cell_data, cmip6_overview, id_name):
    ...
    cmip_to_shed_loc.insert(0,id_name,cell_data[id_name])
...
if __name__ == "__main__":
    ...
    country = "Nepal"   # Madagascar, Nepal
    id_name = "fid"  # fs_uid, fid
    basedir = "/home/christina/Data/csph"
    filename = f'{basedir}/shape_files/centroids_healthsheds_{country}.csv'

    cmip6_overview_file = f'{basedir}/CMIP6/cmip6-zarr-consolidated-stores.csv'
    ...
    cmip6_models = ['MPI-ESM1-2-HR',  # (0.9 x 0.9) Germany
                    ...
                    # 'HadGEM3-GC31-MM',  # (0.83 x 0.56) UK   # error: invalid date
    ...
    for source_id in cmip6_models:
        ...
        if not os.path.exists(f"{basedir}/CMIP6/{country}/{source_id}"):
            os.makedirs(f"{basedir}/CMIP6/{country}/{source_id}")
        ...
        for experiment_id in experiments:
            ...
            for my_var, my_table_id in my_variables:
                ...
                member_ids = member_ids[:5]

                for member_id in member_ids:
                    print(f"\t\t\tMember: {member_id}")

                    if my_table_id == "fx":
                        result_file = f"{basedir}/CMIP6/{country}/{source_id}/{source_id}_{my_var}.csv"
                    else:
                        result_file = (f"{basedir}/CMIP6/{country}/{source_id}/"
                    ...
                    cell_data = pd.DataFrame({
                        'lat': lat.to_xarray(),
                        'lon': lon.to_xarray(),
                        id_name: sheds[id_name]
                    })

                    result = get_cmip6(my_var, my_table_id, source_id, experiment_id, member_id,
                                                      start_day, end_day, cell_data, cmip6_overview, id_name)
                    ...
                    # ======================================================================================================
                    df = df.round(8 if my_var == "pr" else 2)  # round to 8 digits for precipitation and to 2 otherwise
                    df.index.name = "date"
                    df.to_csv(result_file, index=True)

                    # Save shed-to-CMIP mapping
                    # ==============================================================================================================
                    file_path_mapping = f"{basedir}/CMIP6/{country}/{source_id}/CMIP_mapping_{source_id}.csv"
